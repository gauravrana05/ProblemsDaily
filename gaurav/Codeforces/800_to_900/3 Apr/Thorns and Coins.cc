...
int recursion(int st, int size, string s)
{
  ...
  if (s[st] == '@')
  {
    ...
    return 1 + max(recursion(st + 1, size, s), recursion(st + 2, size, s));
  }
  else
  {
    return max(recursion(st + 1, size, s), recursion(st + 2, size, s));
  }
}
void solve()
{
