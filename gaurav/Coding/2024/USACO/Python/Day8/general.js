//THIS IS A GENERAL JAVASCRIPT THATS HAS BEEN INCLUDED
// IN THE LINKSLIB/GENERAL  GEN_TOP() FUNCTION

//Used for checking and unchecking of the -->
function check(n)
{
	n=eval("document."+n)
...
function termpop(url,title,w,h) {

		//alert(url+' ' + title+'  ' + w+ ' '+ h); 

		var param = 'width=' + w + ',height=' + h ;
		param += ',resizable=no,scrollbars=no,top=100,left=100';
...
function checkEmailAddress(addr){
		...
		function indexOfAny(s, sCheck, start) {
		  for (ind=start;ind<sCheck.length;ind++) {
			if (s.indexOf(sCheck.charAt(ind)) >= 0) {
			  return ind;
			}
		  }
		  return -1;
		...
		var sReturn = "";
		var sInv = "()<>,;:\\\"[]@"; //invalid characters
		var sWht = " \t\n\r"; //whitespace
		var iAt = addr.lastIndexOf('@');
		var s1, s2;
		var iErr = 0;
		...
		s1 = addr.substring(0,iAt);
		s2 = addr.substring(iAt+1);
		if (indexOfAny(addr,sWht,0) >= 0) {
		  sReturn += "  Illegal whitespace in address\n";
		  iErr++;
		}
		if (s1 == "") {
		  sReturn += "  No local name defined\n";
		  iErr++;
		}
		if (indexOfAny(s1,sInv,0) >= 0) {
		  sReturn += "  Invalid character - " + sInv + " - in local name\n"
		  iErr++;
		}
		var iDot = s2.lastIndexOf('.');
		if (iDot == s2.length - 1 && s2.length > 0) {
		  sReturn += "  Illegal '.' at end of address\n";
		  iErr++;
		}
		else if (iDot < 0  && s2.length > 0) {
		  sReturn += "  No '.' in address domain\n";
		  iErr++;
		}
		if (s2 == "") {
		  sReturn += "  No address domain defined\n";
		  iErr++;
		}
		if (indexOfAny(s2,sInv,0) >= 0) {
		  sReturn += "  Invalid character - " + sInv + " - in address domain\n";
		  iErr++;
		}
		return iErr == 0 ? 0 : 1 ;
}
